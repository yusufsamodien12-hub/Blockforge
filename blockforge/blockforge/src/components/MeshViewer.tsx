  ...
  useEffect(() => {
    ...
    const buildMeshes = async () => {
      ...
      for (const part of spec.parts) {
        ...
        // Per-part textureUrl (from BlockForge worker's applyTextureUrls)
        // takes priority when available. Falls back to the global textureMaps
        // from asset search, then to flat color.
        if (part.material.textureUrl) {
          try {
            const tex = await loadTexture(part.material.textureUrl);
            const tiled = configureTile(tex, footprint, true);
            materialOptions.map = tiled;
            loadedTexturesRef.current.push(tiled);
          } catch {
            // Texture failed to load - fall through to other options below
          }
        }
        if (!materialOptions.map && baseMaps.diffuse) {
    ...
    <div ref={containerRef} className="mesh-viewer">
      {isLoading && (
        <div className="viewer-overlay">
          ...
          <div className="overlay-label">Researching material &amp; shape</div>
        </div>
      )}
      {!spec && !isLoading && (
